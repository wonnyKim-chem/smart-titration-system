...
DuckDNS 같은 무료 DDNS와 Let’s Encrypt 공인 인증서를 사용할 수 있습니다. Let’s Encrypt 기본 인증서의 유효기간은 24시간이 아니라 **90일**이며 보통 60일마다 자동 갱신합니다. DuckDNS TXT API와 ACME DNS-01 인증을 사용하면 서버 포트를 인터넷에 공개하지 않고도 인증서를 발급할 수 있습니다.

단, 인증서 발급과 실제 접속은 별개입니다. `이름.duckdns.org`가 공인 IP를 가리키는 일반 구성에서는 공유기의 NAT loopback과 포트 전달이 필요하고 서버가 외부에 노출될 수 있습니다. 로컬 전용으로 유지하려면 같은 Wi-Fi에서 해당 도메인이 서버 PC의 사설 IP로 해석되도록 공유기의 로컬 DNS 재정의(분할 DNS)가 필요합니다. 이 조건을 갖추면 모바일 기기에 로컬 CA를 설치하지 않고 공인 인증서를 사용할 수 있습니다.

DuckDNS 토큰은 도메인 DNS를 변경할 수 있는 민감정보입니다. 앱에 연동할 경우 Windows 자격 증명 관리자에 저장하고 로그·설정 파일에 기록하지 않아야 합니다. 현재 배포본은 인터넷이나 공유기 설정 없이 동작하는 로컬 CA 방식을 기본으로 사용합니다.
